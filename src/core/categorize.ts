//--------------------------------------------------------------
// FILE: src/core/categorize.ts
// Character Thread Categorizer — 14 AI slots
//--------------------------------------------------------------
...
import { color, CYAN, GREEN, YELLOW, MAGENTA, RESET } from "../ui/colors.js";
...
function saveProgress(
  ...
  console.log(color(`\n💾 Progress saved! Resume anytime by running the categorizer again.`, GREEN));
}
...
async function main() {
  ...
  console.log(color("║    Sort Conversation Threads by AI        ║", CYAN));
  console.log(color("╚════════════════════════════════════════════╝\n", CYAN));
  ...
  try {
    ...
    // Write output files
    const outDir = path.join(process.cwd(), "categorized");
    ...
    for (const key of AI_KEYS) {
      ...
      if (threads.length > 0) {
        ...
        console.log(color(`✅ Wrote ${threads.length} threads → categorized/ai${num}.json`, GREEN));
